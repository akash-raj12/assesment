...
collection_queries.Register = (data) => {
    return new Promise((resolve, reject) => {
        try {
            connection.query(`select username from  akashk where username='${data.username}'`, (err, rows, fields) => {
                ...
                if (rows.length > 0) {
                    ...
                    if (data.type == 'admin' || data.type == 'customer' || data.type == 'sp') {
                        connection.query(`insert into akashk (type,username,phone,email,password) values ('${data.type}','${data.username}','${data.phone}','${data.email}','${data.password}')`, (err, rows, fields) => {
                            if (!err) {
...
collection_queries.Login = (type, username) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            connection.query(`select password from  akashk where type='${type}' and username='${username}'`, (err, rows, fields) => {
...
collection_queries.Update = (type, username, latitude, longitude) => {
    return new Promise((resolve, reject) => {
        try {
            connection.query(`update akashk set latitude='${latitude}',longitude='${longitude}' where type='${type}'and username='${username}'`, (err, rows, fields) => {
                if (!err) {
                    resolve(rows);
                }
                else {
...
var ori_lon =orig_longitude ;
    ...
    return new Promise((resolve, reject) => {
        ...
        connection.query(`select latitude,longitude from akashk where type='admin'`, (err, rows, fields) => {
            for (var i of rows) {
        ...
        PI()/180/2
        ),2))) as distance from akashk where type='${type}' having distance<'${distance}' `, (error, rows, field) => {
                        if (error) {
...
collection_queries.Fetch = (type, username, fetchOf) => {
    ...
    return new Promise((resolve, reject) => {
        if (type == 'admin' || type == "Admin") {
            connection.query(`select * from akashk where type='${fetchOf}'`, (err, rows, fields) => {
